"""
Open-Close TFNBS (E, H) sensitivity sweep — array-mode implementation.

Sweeps a 6×6 grid of (E, H) values inside the TFNBS family on the
China cohort paired Open vs Close (run 0) contrast.

Why China? Smaller cohort (n=48 paired vs IHB n=84) → faster permutation
pass at no cost to the sensitivity-characterisation claim, which only
needs a non-trivial paired signal to characterise.
...
then exponentiates them per (E, H) cell.
...
def _make_eh_panels(
    ...
    """6-panel (E, H) sensitivity figure (matches ABIDE layout)."""
    ...
    fig.suptitle(
        "TFNBS (E, H) sensitivity on Open-Close China cohort "
        "(paired sign-flip, eyes-open vs eyes-closed run 0)\n"
...
def main():
    ...
    PMAPS_DIR.mkdir(parents=True, exist_ok=True)
    ...
    result = compute_p_val(
        ...
        e=list(e_arr), h=list(h_arr), n=10,
    ...
    _make_eh_panels(grid_summary, spr_pos_df, spr_neg_df, str(plot_path))
    print(f"\nFig saved: {plot_path}")
