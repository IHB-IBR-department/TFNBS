...
def main():
    ihb = np.load(RESULTS / "ihb_paired_tfnbs.npz"); china = np.load(RESULTS / "china_paired_tfnbs_run0.npz")
    ...
    for r, tail in enumerate(["positive", "negative"]):
        ma = _block_mass(ihb[tail], lab); mb = _block_mass(china[tail], lab); vmax = max(ma.max(), mb.max())
        for c, (M, title) in enumerate([(ma, f"IHB {tail}"), (mb, f"Beijing China {tail}")]):
    ...
    plt.suptitle("Block-Mass Matrix Convergence: Cross-Cohort Replication (TF-NBS: E=0.4, H=3.0)\nComparison of topological integration patterns (Σ −log10 p) between IHB and Beijing China", fontsize=16)
