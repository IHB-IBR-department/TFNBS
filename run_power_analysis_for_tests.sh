...
# Лог и PID файлы
LOG=/projects/TFNBS/.log/TEST_power_analysis_short_$(date +%d%m%H%M).log
PIDFILE=/projects/TFNBS/.log/TEST_power_analysis_short_$(date +%d%m%H%M).pid

# Удаляем старые (если есть)
rm -f $PIDFILE $LOG

# Запуск в фоне
nohup taskset -c 48-63 \
  python -u examples/power_test/power_analysis.py \
  --config examples/power_test/sweep_config_power_server_short.yaml \
  > $LOG 2>&1 &
