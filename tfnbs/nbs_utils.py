...
def nbs_bct(group1: npt.NDArray[np.float64],
    ...
    """
    Function to compute Network-Based Statistics (NBS) of individual (One sample, Two sample) and independent (Paired) groups of connectivity matrices using a fixed t-statistic threshold.
    ...
    Parameters:
        ...
        test_type (str): String indicating the type of statistical T-test to be performed
            Should be one of ['paired', 'one-sample', 'two-sample'] (default = 'paired')*.
    ...
    *When test_type = 'one-sample', group2 field is optional and can be left empty. 

    >>> g1 = np.random.rand(10, 5, 5)
    >>> g2 = np.random.rand(10, 5, 5)
    >>> p_vals, adjs, nulls = nbs_bct(g1, g2, threshold=1.5, n_permutations=100, test_type='paired', use_mp=False, random_state=0) 
    ...
    if test_type == 'paired':
        ...
        emp_t_dict = compute_t_stat(group1, test_type = 'one-sample')
    ...
    if test_type == 'paired' or test_type == 'two-sample':
        ...
        max_null_dict = compute_null_dist(group1,
                                        ...
                                        **kwargs)
        p_values = []
        p_values = np.mean(emp_t < max_null_dict, axis=-1)
