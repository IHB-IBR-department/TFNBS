...
class TestRecipeSmoke(unittest.TestCase):
    ...
    def test_h0_recipe_runs_clean(self):
        ...
        # PR-2 + PR-3 + PR-4 provenance triple lit up by sites=
        ...
        # Auto-strata and auto-preserve flags both present
        joined = "\n".join(out.flags)
        self.assertIn("strata= auto-set", joined,
                      f"missing auto-strata flag in {out.flags}")
        self.assertIn("preserve auto-built", joined,
                      f"missing auto-preserve flag in {out.flags}")

        # ComBat actually reduced between-site variance on the planted shift
        ratio = out.combat_diagnostics.get(
            "between_site_variance_ratio_after_over_before"
        )
        if ratio is None:
            # Some diagnostics blob versions use the *_after / *_before
            # split rather than the explicit ratio key
            r_after = out.combat_diagnostics["between_site_variance_ratio_after"]
            r_before = out.combat_diagnostics["between_site_variance_ratio_before"]
            ratio = r_after / r_before
        self.assertLess(
            ratio, 0.5,
            msg=f"ComBat didn't visibly reduce site variance (ratio={ratio:.3f})",
        )
